...
const EmailConfigManager = () => {
  ...
  // isPending works in both v4 (isLoading) and v5 (isPending)
  const isAdding   = addMutation.isPending   ?? addMutation.isLoading;
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 mb-6">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 disabled:opacity-60 disabled:cursor-not-allowed transition-opacity"
              disabled={isAdding}
            />
            <button
              onClick={handleAddEmail}
              disabled={isAdding}
              className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2 min-w-[90px] justify-center"
            >
              {isAdding
          ...
          {/* Inline loading feedback below input */}
          {isAdding && (
            <p className="mt-2 text-xs text-blue-500 dark:text-blue-400 flex items-center gap-1.5">
              <Loader2 className="animate-spin" size={11} />
              Saving email, please wait...
            </p>
          )}
        </div>
